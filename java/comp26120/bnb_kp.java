...
public class bnb_kp extends KnapSack {
    ...
    struc_sol temp_final_sol = new struc_sol();
    frac_bound(temp_final_sol, 0);
    int current_best = temp_final_sol.val;
    double best_bound = temp_final_sol.bound;
    insert(temp_final_sol);
     
    while(QueueSize != 0 && temp_final_sol.bound > current_best){

        struc_sol temp_struct = removeMax(); 
        struc_sol temp_struct_1 = temp_struct.copy();
        struc_sol temp_struct_2 = temp_struct.copy();

        temp_struct_1.fixed++;
        temp_struct_2.fixed++;
        temp_struct_1.solution_vec.set(temp_struct_1.fixed, true);
        temp_struct_2.solution_vec.set(temp_struct_2.fixed, false);
       // best_bound = temp_struct.bound; 

        if(check_evaluate_and_print_sol(temp_struct_1.solution_vec) == 0){
            frac_bound(temp_struct_1, temp_struct_1.fixed);
        
           // if(temp_struct_1.bound >= best_bound){
                if(temp_struct_1.val > current_best){
                    current_best = temp_struct_1.val;
                    temp_final_sol = temp_struct_1.copy();
                }
                insert(temp_struct_1);
            //}
        }

        if(check_evaluate_and_print_sol(temp_struct_2.solution_vec) == 0){
            frac_bound(temp_struct_2, temp_struct_2.fixed);

           // if(temp_struct_2.bound >= best_bound){
                if(temp_struct_2.val > current_best){
                    current_best = temp_struct_2.val;
                    temp_final_sol = temp_struct_2.copy();  
                }
                insert(temp_struct_2);     
          // }
        }
    }

    copy_array(temp_final_sol.solution_vec, final_sol);  
